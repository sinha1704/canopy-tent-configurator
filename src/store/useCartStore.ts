...
export const useCartStore = create<CartState>((set) => ({
  ...
  addToCart: (thumbnailSnapshotBase64 = '') => {
    ...
    const refId = `MVP-CANOPY-${Date.now().toString(36).toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    const newPayload: ShopifyCartPayload = {
      variantId,
      sku: `MVP-TENT-${cfg.tentSize.replace('.', '')}-${cfg.frameType === 'hex_aluminum_pro' ? 'HEX' : 'STL'}`,
      ...
      properties: {
        ...
        _pdf_spec_url: `https://orders.mvpvisuals.com/specs/${refId}.pdf`
      },
